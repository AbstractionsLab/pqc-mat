...
def main() -> int:
    ...
    print(f"  Annotated CBOM written to: {output_path}")

    if not args.no_report:
        report_path = args.report or _default_report_path(input_path)
        report_text = generate_report(scored)
        with open(report_path, "w", encoding="utf-8") as f:
            f.write(report_text)
        print(f"  Risk report written to:    {report_path}")
